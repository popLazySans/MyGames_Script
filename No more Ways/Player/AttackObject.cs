   ...
   [ServerRpc (RequireOwnership =false)]
    ...
    private GameObject CheckEnemyTarget()
    {
        ...
        foreach (GameObject player in movementObject.playerArray)
        {
            ...
            if (movementObject.playerMove.playerPosition[0] + movementObject.moveToPosition[0] == othermove.retrunThisPosition()[0]
                && movementObject.playerMove.playerPosition[1] + movementObject.moveToPosition[1] == othermove.retrunThisPosition()[1])
            {
                target = player;
            }
            else
